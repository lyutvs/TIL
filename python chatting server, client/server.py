...
HOST = '10.156.147.207' # 서버의 ip를 열음. (이 서버의 ip로 클라이언트가 접속을 해야 한다), 그전에 ping을 먼저 확인하도록.
PORT = 9009				 # 포트번호 (같아야 함)
lock = threading.Lock()  # syncroniszed 동기화 진행하는 스레드 생성
class UserManager:  # 사용자관리 및 채팅 메세지 전송을 담당하는 클래스
    ...
    def addUser(self, username, conn, addr):  # 사용자 ID를 self.users에 추가하는 함수
        if username in self.users:  # 이미 등록된 사용자라면, 이름 같으면 출력
        ...
        return username
    def removeUser(self, username):  # 사용자를 제거하는 함수, 퇴장 함수
